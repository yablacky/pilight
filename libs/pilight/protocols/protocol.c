...
void protocol_init(void) {
	...
	static const char pilight_version[] = PILIGHT_VERSION;
...
void protocol_register(protocol_t **proto) {
	...
	CONFIG_ALLOC_UNNAMED_NODE(*proto);
	(*proto)->options = NULL;
	(*proto)->devices = NULL;
	...
	CONFIG_PREPEND_NODE_TO_LIST(pnode, protocols);
}

struct protocol_threads_t *protocol_thread_init(protocol_t *proto, struct JsonNode *param) {
	logprintf(LOG_STACK, "%s(...)", __FUNCTION__);

	struct protocol_threads_t *node = MALLOC_OR_EXIT(sizeof(struct protocol_threads_t));
...
void protocol_set_id(protocol_t *proto, const char *id) {
	...
	proto->id = STRDUP_OR_EXIT(id);
}

void protocol_device_add(protocol_t *proto, const char *id, const char *desc) {
	logprintf(LOG_STACK, "%s(...)", __FUNCTION__);

	struct protocol_devices_t *dnode = NULL;
	CONFIG_ALLOC_UNNAMED_NODE(dnode);
	dnode->id = STRDUP_OR_EXIT(id);
	dnode->desc = STRDUP_OR_EXIT(desc);
